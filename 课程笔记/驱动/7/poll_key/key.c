...
void fs4412_key_timer_handler(unsigned long data)
{
	...
	wake_up_interruptible(&fs4412_keyq);
	flag = 1;
...
ssize_t fs4412_key_read(struct file *filp,char __user *ubuf,size_t size,loff_t *off)
{
	...
	wait_event_interruptible(fs4412_keyq,flag != 0);
...
unsigned int fs4412_key_poll(struct file *filp,struct poll_table_struct *p)
{
	unsigned int mask = 0;

	poll_wait(filp,&fs4412_keyq,p);
