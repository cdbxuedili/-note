...
irqreturn_t fs4412_key_handler(int irqno,void *dev_id)//中断上半部
{
	...
	tasklet_schedule(&task);//通知下半部可以开始执行了
	schedule_work(&work);
	return IRQ_HANDLED;
}

void task_handler(unsigned long data)//中断下半部,下半部要想执行至少要等上半部执行完
{
	printk("tasklet 下半部 done\n");
}

void work_handler(struct work_struct *work)//工作队列的下半部函数
{
	printk("workqueue 下半部 done\n");	
}
...
int fs4412_key_probe(struct platform_device *pdev)
{
	...
	tasklet_init(&task,task_handler,0);//此时设置了中断下半部的入口
