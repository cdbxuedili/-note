...
int main(void) {
	...
	PWM .TCFG1 = (PWM .TCFG1 & ~(0XF << 4)) | (0B0010 << 4); // timer0 的工作频率 1M / 4 = 250Khz 每秒计数250000 次

	PWM .TCNTB1 = 250 - 1; // 计数250次  总周期1ms 频率 1Khz
	PWM .TCMPB1 = 250 / 2; // 占空比50%

	PWM .TCON = (PWM .TCON & ~(0XF << 8)) | (0B1010 << 8); // DEAD = 0 RELOAD = 1 Inverter = 0  UPDATE = 1 START = 0
	PWM .TCON = (PWM .TCON & ~(0XF << 8)) | (0B1000 << 8); // DEAD = 0 RELOAD = 1 Inverter = 0  UPDATE = 0 START = 0
	PWM .TCON = (PWM .TCON & ~(0XF << 8)) | (0B1001 << 8); // DEAD = 0 RELOAD = 1 Inverter = 0  UPDATE = 0 START = 1

	GPD0 .CON = (GPD0 .CON & ~(0XF << 4)) | (0X2 << 4);  // 选择tout0 输出模式
												//	0x0 = Input
												//	0x1 = Output
												//	0x2 = TOUT_0
	...
	while (1) {
		GPX1 .DAT =  (GPX1 .DAT & ~1) | ((GPD0.DAT >> 1) & 1 );
	}
